...
All information regarding contributing to and progressing `ethjs-format` module can be found in this document.
...
You can find the coverage report and view the percentages and stats, by going to the [index.html](coverage/lcov-report/index.html) file generated after running the script. Open this in Chrome to see the generated report.
...
Across all `ethjs-` repos, we enforce mandatory autmated testing. We never `commit to $DEFAULT_BERANCH`. As a general policy, test-coverage results must always be above 95% for any `ethjs-` PR or commit. We want to ensure complete coverage across the board.

## Contributing

Please help better the ecosystem by submitting issues and pull requests. We need all the help we can get to build the absolute best linting standards and utilities. We follow the AirBNB linting standard. Please read more about contributing to `ethjs-format` in the `.github/CONTRIBUTING.md`.
